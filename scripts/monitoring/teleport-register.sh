...
if ! kubectl get svc kube-prometheus-stack-grafana -n "${MONITORING_NAMESPACE}" >/dev/null 2>&1; then
    echo "Error: Grafana service not found in namespace '${MONITORING_NAMESPACE}'."
    echo "Run ./scripts/monitoring/install.sh first."
    exit 1
fi
